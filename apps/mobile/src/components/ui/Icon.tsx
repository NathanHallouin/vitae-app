/**
 * Le jeu d'icônes du site, redessiné avec `react-native-svg`.
 *
 * Les tracés sont copiés à l'identique : même grille de 24 px, même trait de 1,6, mêmes courbes.
 * Seules les balises changent, `react-native-svg` n'ayant pas de DOM. Les icônes ne portent jamais
 * le sens seules — un libellé les accompagne toujours — donc elles sont masquées aux lecteurs
 * d'écran, sauf `title` explicite.
 *
 * `currentColor` n'existe pas en React Native : la couleur du trait est passée en propriété. Les
 * appelants la tirent du thème via `useToken`, ce qui revient au même à l'écran.
 */

import { type IconName, ICON_NAMES } from '@vitae/core/icons';
...
export default function Icon({
  ...
  name: IconName;
  size?: number;
  /** couleur du trait ; par défaut la couleur de texte courante du thème */
  color?: string;
